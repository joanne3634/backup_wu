...
public class TcprojectService extends BaseService<Tcproject>{
			...
			for (Tcproject t : list) {
				...
				if(StringUtil.isNotEmpty(t.getPstateId())){
					if(t.getPcheckTimes()<=1){//审核次数小于2次的,初次审核为0
						if(t.getPstateId().equals("1")||t.getPstateId().equals("2")){
	...
	public List<Tcproject> getAllHotProjects(){
		List<Tcproject> list=tcprojectDao.getAllHotProjects();
		return list;
	}
			...
			for (Tcproject t : list) {
				if(t.getPisFull()==0){
					t.setLinkedAction("<a href=\"javascript:void(0)\" class=\"link primary\" onclick=\"agreeApply("+t.getProjectId()+",'agree');\">满标</a>&nbsp;&nbsp;&nbsp;&nbsp;"+
							"<a href=\"get.ht?projectId="+t.getProjectId()+"\" target=\"_blank\"  class=\"link resetPwd\" >流标</a>");
				}else{
	...
	public Tcproject getTcprojectById(long proId){
		return tcprojectDao.getTcprojectById(proId);
	}
	...
	public boolean calculateProjectInvestmentInfo(Long projectId){
		...
		try{
			...
			if(listIn!=null&&listIn.size()>0){
				...
				BigDecimal value = new BigDecimal(0);//估值
				int asks = 0;
				int investCount = listIn.size();
				List<String> listSort=new ArrayList<String>();
				for(TcprojectInvest invest:listIn){
					/*if(invest.getPiisAskPrice()>0){ 
						if(value.intValue()==0){
							value = invest.getPivaluation();
						}else if(value.compareTo(invest.getPivaluation())>0){
							value = invest.getPivaluation();
						}
					}*/
					////将投资人的估值 更新到项目表的 项目最终估值字段 如果有的话
					//如果 创业者已经同意 投资人的 询价估值 ，就将投资人的估值 更新给 项目表的最终估值
					//项目的估值 遵守这样的原则, 50 100 150  三位数字 ，如果投资者 估值 50 创业者 同意了,则项目的最终估值 为50,再有 投资者 估值 100 ，也按最小的估值计算.
					//总之 项目估值 尊 创业者 同意的 最小估值 计算   2/11 by pyg
					
					
					//找出 最小的估值 
					//4/17 客户要求去掉询价功能
					/*if(invest.getPiisAskPrice()>0&&invest.getPiisCheck()>0&&invest.getPiisCancel()==0){// 投资者询价 且 经 创业者同意 
						if(invest.getPivaluation()!=null)
							listSort.add(invest.getPivaluation()+"");
					...
					/*if(invest.getPiisAskPrice()>0){//如果是询价记录 // 4/17 客户要求去掉询价功能
						asks++;
					}*/
				}
				/*if(listSort!=null&&listSort.size()>0){ // 4/17 客户要求去掉询价功能
					Collections.sort(listSort);
					if(StringUtil.isNotEmpty(listSort.get(0))){
						value=new BigDecimal(listSort.get(0));
					}
					tcproject.setPvaluationFinal(value);
				}else{
					tcproject.setPvaluationFinal(value);
				}*/
