...
import com.deelon.crowdfunding.service.backstage.TcprojectNewsService;
...
public class FinancingProjectByWhereController extends BaseController {
	...
	public ModelAndView tofinancingProList(HttpServletRequest request,
		...
		String pstage = request.getParameter("pstage");//项目阶段:1众筹中2融资成功
		...
		try {
			...
			if(StringUtil.isNotEmpty(pstage)){
				if(Pstage1Constrant.equals(pstage)){
					pstage = "(1,2,3)";
					map.put("pstage",pstage); 
				} else if(Pstage2Constrant.equals(pstage)){
					pstage = "(4,5,6)";
					map.put("pstage", pstage);
				}
			}
	...
	public ModelAndView toProDetails(HttpServletRequest request,
		...
		try {
			...
			if(tcproject!=null){
						//根据项目id查询投资的估值最小值
						/*TcprojectInvest tcprojectInvest = tcprojectInvestService.getMinValueByProId(tcproject.getProjectId());
						if(tcprojectInvest!=null){
							if(!tcprojectInvest.getPivaluation().equals("0.000")){
								BigDecimal sharePercent = tcproject.getPfinancingAmount().divide(tcprojectInvest.getPivaluation(), 2,RoundingMode.HALF_UP).multiply(new BigDecimal(100));
								tcproject.setPsharesPercent(sharePercent);
							}else{
								tcproject.setPsharesPercent(new BigDecimal(0.00));	
							}
						}*/
				//4/17 因为客户要求去掉询价功能,项目估值有创业者和客户运营方 确定,这里的项目估值为项目表的最终估值字段
		...
		//	invest.put("userId", userId);
			...
			//取得系统设置的项目融资上限
			 ...
			 //判断用户有无绑定银行卡,
			 if(u!=null){
				 List<TBankcard> lCar=tbankcardService.getTBankcardByUserId(u.getUserId());
				 if(lCar==null||lCar.size()==0){
					 request.setAttribute("IsBindCard", "1");
				 }
			 }
			 //平台设置的项目认筹诚意金
	 ...
	 @RequestMapping({"toProComment"})
	  public ModelAndView toProComment(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			//项目评论
			List<TcprojectComments> listC=tcprojectCommentsService.getTcprojectCommentsByProId(map,null);
...
//		 String pitypeId=request.getParameter("pitypeId");//客户要求 去掉询价功能 领投人 按领投部分算，
		 String piisGeneralPartner=request.getParameter("piisGeneralPartner");
		 
		 String piisLeader=request.getParameter("piisLeader");
	///	 String yesAsk=request.getParameter("yesAsk");//客户要求 去掉询价功能
		...
		try {
			...
			if(StringUtil.isNotEmpty(piisLeader)){
				if(piisLeader.equals("1")){//领投人
					...
					/*if(pitypeId.equals("1")){
						tcprojectInvest.setPiinvest(tcproject.getPfinancingAmount());//全部认投
					}else if(pitypeId.equals("2")){*/
						if(StringUtil.isNotEmpty(piinvest))
							tcprojectInvest.setPiinvest(new BigDecimal(piinvest).multiply(new BigDecimal(10000)));
						
			//		}
						//		tcprojectInvest.setPitypeId(Integer.valueOf(pitypeId));
	...
	//		if(StringUtil.isNotEmpty(pivaluation)){
		...
		//	}
			...
			//询价次数 字段 应在 创业者审核通过后更新
		//	tcprojectInvest.setPiisAskPrice(Integer.valueOf(yesAsk));
	 ...
	 @RequestMapping({"addtoInvestMoney"})
	  public ModelAndView addtoInvestMoney(HttpServletRequest request, HttpServletResponse response)
	    throws Exception {
		 ...
		 String projectId=request.getParameter("projectId");
		 //String pfinancingAmount = request.getParameter("pfinancingAmount");
		 Tcproject cp= tcprojectService.getById(Long.valueOf(projectId));
		//超募比例
		 BigDecimal recruitment=new BigDecimal((String)AppUtil.getServletContext().getAttribute("RECRUITMENT_LIMIT_FINANCING")).divide(new BigDecimal(100));
		 //项目可投金额=项目总额*1.2-项目已投金额
		 BigDecimal allowInvest = cp.getPfinancingAmount().multiply(recruitment).subtract(cp.getPinvestAmount());
		 String pfinancingAmount=allowInvest+"";
		 request.setAttribute("projectId", projectId);
...
//				 Tcproject cp= tcprojectService.getById(Long.valueOf(projectId));
//				 pfinancingAmount=cp.getPfinancingAmount()+"";
//			 }
		 ...
		 return  new ModelAndView("/crowdfunding/front/addtoInvestMoney.jsp").addObject("askPriceNum", askPriceNum).addObject("pfinancingAmount", pfinancingAmount);
	 }
