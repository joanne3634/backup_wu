...
<head>
   ...
   <script type="text/javascript" src="${ctx}/js/deelon/platform/system/FlexUploadDialog.js"></script>
	<script type="text/javascript">
			...
			if(agOrdisag=='agree'){
				...
				$.ligerDialog.confirm("确认执行该操作吗？","提示信息",function(rtn){
					if(!rtn){
						...
						if(isok==1){
							$.ligerDialog.warn("项目的信息并没有填写完整，请联系项目创建人完成项目信息再审核通过。详情请点击<a href='edit.ht?projectId="+pamrm+"' class='link edit'>查看创业者资料</a>。");
						}else{
							var rep=$.ajax({  
						        type: "POST",  
	 ...
	 function recommandProject(id,isrecomm){
		 var rep=$.ajax({  
		        type: "POST",  
		        url: "${ctx}/crowdfunding/backstage/proAudit/updates.ht",
		        data: {applyIds:id,isrecomm:isrecomm }  
		        });  
			rep.done(function( data ){  
				 $.ligerDialog.warn(data.msg);
		        });  
		...
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="group"><a class="link back" href="${ctx}/crowdfunding/tcprojectAfterInfo/list.ht"><span></span>返回</a></div>
					</c:if>
					<c:if test="${flag eq ''||flag==null}">
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link back"
					<c:if test="${empty IsShowBtn }">href="list.ht"</c:if> 
	           ...
	           <div title="项目详情" tabid="userdetail" icon="${ctx}/styles/default/images/resicon/user.gif">
			         <table class="table-detail" cellpadding="0" cellspacing="0" border="0">
							<tr>
								...
								<th width="25%">项目状态:</th>	
								<td>	
								 <f:select  disabled="true"  nodeKey="xmzt" id="Q_pstage_S" defaultValue="${tcproject.pstateId}"></f:select> 								 
							...
							<tr>
								...
								<td>										 
									...
									<textarea rows="2" cols="1" readonly="readonly" id="paddress" name="paddress">${tcproject.paddress}</textarea>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<th width="25%">项目估值:</th>	
								<td>										 
									<input readonly="readonly"  readonly="readonly"  type="text" id="pdetail" name="pdetail" value=" <fmt:formatNumber value="${tcproject.pvaluationFinal}" pattern="###,###"  minFractionDigits="0"  />" class="inputText" />万 
							...
							<tr>
								...
								<td colspan="4">										 
									...
									<textarea rows="2" cols="1" readonly="readonly" id="pdetail" name="pdetail">${tcproject.pdetail}</textarea>
								</td>
