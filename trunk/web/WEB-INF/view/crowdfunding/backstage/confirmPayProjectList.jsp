...
<body>		
					...
					<div class="panel-search">
						<form id="searchForm" method="post" action="list.ht">
							<div class="row">
									...
									<span >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> 
									<span class="label">项目类别:</span><f:select nodeKey="XMLX" showBlankOnTop="true" id="Q_ptypeId_S" defaultValue="${param['Q_ptypeId_S']}"></f:select> 
							</div>
							<div class="row">
									<span class="label">所属行业:</span><f:select nodeKey="PIND" showBlankOnTop="true" id="Q_pindustry_S" defaultValue="${param['Q_pindustry_S']}"></f:select> 
					...
					<div class="panel-data">
					    ...
					    <display:table name="tprojectList" id="tprojectItem" requestURI="list.html" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
							...
							<display:column  title="项目名称" sortable="false" sortName="pname" style="text-align:left">
								<a title="点击查看" href="${ctx }/crowdfunding/backstage/proAudit/edit.ht?projectId=${tprojectItem.projectId}&IsShowBtn=${ctx}/crowdfunding/backstage/confirmPayProject/list.ht" >${tprojectItem.pname }</a>
							</display:column> 
							...
							<display:column title="操作" media="html"  class="rowOps">
								<c:if test="${tprojectItem.ppayPercent!=100}">
									<%-- <c:if test="${tprojectItem.pisPay==2}"><a href='cacelPay.ht?projectId=${tprojectItem.projectId}&ispay=2'  class='link edit'>恢复</a> 
																		   <a  href='cacelPay.ht?projectId=${tprojectItem.projectId}&ispay=3'  class='link edit'>取消</a></c:if> --%>
									<%-- <c:if test="${tprojectItem.pisPay!=2}"> --%>
									<%-- <a target="_blank" href='editPay.ht?projectId=${tprojectItem.projectId}' class="link edit">放款</a> --%>
									
									 <a href='javascript:void(0)' onclick="doLoanInfo('${tprojectItem.projectId}')" class="link edit">登记放款信息</a>
									<%-- <a href='cacelPay.ht?projectId=${tprojectItem.projectId}'  class='link edit'>暂停</a> --%>
									<%-- </c:if> --%>	
								</c:if>													
							</display:column>
						</display:table>
 ...
 function doLoanInfo(projectId){
		var url="${ctx}/crowdfunding/backstage/confirmPayProject/loanInfo.ht?projectId="+projectId;
		window.parent.openDialog(url,'项目放款登记','','400','750');
	}
	 $("input[id^='Q_pfinancingAmount2_S']").keydown(function () {  
	 ...
	 $("input[id^='Q_pfinancingAmount1_S']").keydown(function () {  
		    var e = $(this).event || window.event;  
		    var code = parseInt(e.keyCode);  
		    if (code >= 96 &&code <= 105 || code >= 48 && code <= 57 || code == 8||code == 110||code == 190) {  
		        return true;  
