...
function agreeApply(pamrm,agOrdisag){
	...
	if(agOrdisag=='agree'){
		...
		var rep=$.ajax({  
		        type: "GET",  
		        url: "${ctx}/crowdfunding/backstage/expiredProject/viewInvestOfPro.ht?projectId="+pamrm 
		        });  
			rep.done(function( data ){  
					if(data=='1'){
						$.ligerDialog.warn("该项目还有投资人未打款,暂时不能做满标操作,详情请【查看项目投资人】!");
						...
						$.ligerDialog.confirm("确认执行该操作吗？","提示信息",function(rtn){
							if(rtn){
								var rep=$.ajax({  
							        type: "POST",  
							        url: "${ctx}/crowdfunding/backstage/expiredProject/updates.ht",
...
function doOpenInvestWin(projectId){
	var url="${ctx}/crowdfunding/backstage/expiredProject/invest.ht?projectId="+projectId;
	window.parent.openDialog(url,'项目跟投人员记录','','400','750');
}

function extension(pamrm,agOrdisag)
{
	if(agOrdisag=='ext'){
		//延期
		var rep=$.ajax({  
		        type: "GET",  
		        url: "${ctx}/crowdfunding/backstage/expiredProject/list.ht?projectId="+pamrm 
		        });  
			rep.done(function( data ){  
					/* if(data=='1'){
						$.ligerDialog.warn("该项目还有投资人未打款,暂时不能做满标操作,详情请【查看项目投资人】!");
		...
		window.location.target="_blank";
		location.href="edit.ht?projectId="+pamrm;
	}
}
	...
	<div class="panel">
				<div class="panel-top">
					<div class="tbar-title">
						<span class="tbar-label">过期项目列表</span>
					</div>
				...
				<div class="panel-body">
					...
					<div class="panel-data">
					    ...
					    <display:table name="tprojectList" id="tprojectItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1"   class="table-grid">
							...
							<display:column  title="项目名称" sortable="false" sortName="pname" style="text-align:left;width:14%;">
							...
							<display:column  title="满标基准" sortable="false" sortName="setValue" style="text-align:left">
								${tprojectItem.setValue }%
							</display:column>
							...
							<display:column title="操作" media="html" style="width:20%;text-align:left;">
