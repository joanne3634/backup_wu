...
int count2=1;
%>
<form id="query_id_1_" action="${ctx}/loan/tuserMsgRemind/myAboutInfo.ht" method="post">
		...
		<c:if test="${not empty aboutInformInfos }">
		     <c:forEach var="aboutInformInfo" items="${aboutInformInfos}">
			<tr>
			<td><input type="checkbox" name="checkbox" id="checkbox<%=count1++%>" value="${aboutInformInfo.msgRemindId}"/></td>
				<td>
			...
			<c:if test="${fn:length(aboutInformInfo.content) > 25 }">
				<td>${fn:substring(aboutInformInfo.content,0,25) }...</td>
			</c:if>
			<c:if test="${fn:length(aboutInformInfo.content) <= 25 }">
				<td>${aboutInformInfo.content }</td>
			</c:if>
			...
			<td>
			<%-- <img src="${ctx }/crowdfunding/Skin/Default/images/LookIcont.png" title="查看" onclick="detailClick(${aboutInformInfo.msgRemindId});" style="cursor: pointer;">
			<img src="${ctx }/crowdfunding/Skin/Default/images/replyIcont.png" title="约谈" onclick="replyClick('${aboutInformInfo.msgRemindId}','${aboutInformInfo.senderName}');" style="cursor: pointer;">
			<img src="${ctx }/crowdfunding/Skin/Default/images/deleteIcont.png" title="删除" style="cursor: pointer;"  onclick="delRow(${aboutInformInfo.msgRemindId});"> --%>
			
			<a style="cursor: pointer; color: #ff5500;" href="javascript:void(0);" onclick="detailClick(${aboutInformInfo.msgRemindId});">查看</a>
			<a style="cursor: pointer; color: #ff5500;" href="javascript:void(0);" onclick="replyClick('${aboutInformInfo.msgRemindId}','${aboutInformInfo.senderName}');">约谈</a>
			<a style="cursor: pointer; color: #ff5500;" href="javascript:void(0);" onclick="delRow(${aboutInformInfo.msgRemindId});">删除</a>
            </td>
            </tr>
             </c:forEach>
        </c:if>	
