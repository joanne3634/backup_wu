...
<!DOCTYPE html>
  ...
  <link rel="stylesheet" href="${ctx }/styles/p2p/Skin/Default/css/public.css">
   <link rel="stylesheet" href="${ctx }/styles/p2p/Skin/Default/IndustryInformationList.css">
 <script type="text/javascript" src="${ctx}/js/javascripts/jquery-1.11.1.min.js"></script>
        ...
        <div class="con">
          ...
          <c:forEach items="${cms}" var="cms">
            <li> 
               <a href="${ctx}/site/IndustryInformation/IndustryInformationDetails.ht?id=${cms.siteMenuContentId}"><h2>${cms.contentTitle }</h2></a>
                <p class="mediaReport-p">
                   ${cms.contentSummary}
                   </p>
                <span>
                 <c:if test="${!empty cms.contentCreateDate}">
                <fmt:formatDate value="${cms.contentCreateDate }" pattern="yyyy-MM-dd HH:mm"/>
                </c:if>
                </span>
            </li>
            </c:forEach> 
