...
(function ($)
{
    ...
    $.ligerDefaults.Grid.formatters['date'] = function (value, column)
    {
     ...
     //扩展一个 多行文本框 的编辑器
        $.ligerDefaults.Grid.editors['textarea'] = {
            ...
            {
                 var input = $("<textarea class='l-textarea' style='position:absolute;'/>");
                 input.focus(function(){
                 	 var column = editParm.column;
                 	 input.height(column.editor.height);
                 	 input.css({"z-index":99});
                 }).blur(function(){
                 	input.height(container[0]["clientHeight"]-5);
                 	input.css({"z-index":1});
                 })
                input.height(container[0]["clientHeight"]-5);
            ...
            resize: function (input, width, height, editParm)
            {
            	...
            	if(currentExplorer.indexOf("Chrome")>=0||currentExplorer.indexOf("Firefox")>=0){
            		input.css({"margin-left":-Math.ceil(width/2)});
            	}
            	input.css({"margin-top":-Math.ceil(height/2)});
                var column = editParm.column;
                if (column.editor.width) input.width(column.editor.width);
                else input.width(width-6);
            }
        };
        
	     //扩展一个 select弹出框
	    $.ligerDefaults.Grid.editors['selectPos'] = 
	    $.ligerDefaults.Grid.editors['selectRole']=
	    $.ligerDefaults.Grid.editors['selectUser']=
        $.ligerDefaults.Grid.editors['selectOrg'] = 
        $.ligerDefaults.Grid.editors['commonDialog'] =
	    {
	        create: function (container, editParm)
	        {
	            ...
	            var g =this.render(column);
	            g.wrapper.width(column["_width"]);
	            g.inputText.height(container[0]["clientHeight"]-2);
	            g.link.height(container[0]["clientHeight"]-5);
	            g.inputText.css({"top":"-17px"});
	            g.link.css({"top":"-7px"});
	            var currentInput=g.inputText;
	            var currentEditorMulti=column.editor.multi;
	            $(g.link).mousedown(function(){
	            	if(column.editor.type=="selectPos"){
	            		selectPos(editParm.record,column.columnname,currentEditorMulti);
	            	}else if(column.editor.type=="selectRole"){
	            		selectRole(editParm.record,column.columnname,currentEditorMulti);
	            	}else if(column.editor.type=="selectUser"){
	            		selectUser(editParm.record,column.columnname,currentEditorMulti);
	            	}else if(column.editor.type=="selectOrg"){
	            		selectOrg(editParm.record,column.columnname,currentEditorMulti);
	            	}else if(column.editor.type=="commonDialog"){
	            		var options = column.editor.options['dialog'];
	            		CommonDialog(options['name'],function(data){
							var rowdata=editParm.record;
							var fields=options['fields'];
							var currentColumns=[];
							var newRowData={};
							for(var field in fields){
								newRowData[fields[field]['target']]=data[fields[field]['src']];
								currentColumns.push(fields[field]['target']);
							}
	            ...
	            container.append(g.wrapper);
	            return g.wrapper;
	        },
	        getValue: function (input, editParm)
	        { 
	            return $(".l-text-field",input).val();
	        },
	        setValue: function (input, value, editParm)
	        { 
	        	$(".l-text-field",input).val(value);
	        },
	        resize: function (input, width, height, editParm)
	        {
	        ...
	        render: function (column)
	            ...
	            g.inputText[0].name = column['columnname'];
	            //开关
	            g.link = $('<div class="l-trigger" style="height: 21px;top: -2px;"><div class="l-trigger-show-icon">...</div></div>');
	            //外层
	            g.wrapper = g.inputText.wrap('<div class="l-text l-text-popup" style="line-height:10px;"></div>').parent();
           	    g.wrapper.append('<div class="l-text-l"></div><div class="l-text-r"></div>');
	            g.wrapper.append(g.link);
	            g.wrapper.append(g.valueField);
	            g.inputText.addClass("l-text-field");
	            g.inputText.width(column["_width"]);
	            return g;
       		 }
	    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setHeight: function (h)
        {
        	...
        	if(($('.drop').html().indexOf("收起")>=0)){
        	 	if(typeof h!='object'){
        	 		searchDivHeight=$('#searchForm').height();
        	 	}else{
        	 		searchDivHeight=-6;
        	 	}
        	}else{
        		if(typeof h=='object'){
        			searchDivHeight=$('#searchForm').height()-6;
        		}
        	}
        	h="77%";
        ...
        insertValue: function(e){
        	var g = this, p = this.options;
        	var newRowData = $.extend(true, {}, e.rowdata);
        	var newColumnsDatas={_editing:false};
        	for(var col in e.currentColumns){
        		newColumnsDatas[e.currentColumns[col]]=newRowData[e.currentColumns[col]];
        	}
    	 	var newdata = {};
            if (!g.editors[e.rowdata['__id']]) return;
            for (var columnid in g.editors[e.rowdata['__id']])
            {
                var o = g.editors[e.rowdata['__id']][columnid];
                var column = o.editParm.column;
                if (column.name)
                    newdata[column.name] = o.editor.getValue(o.input, o.editParm);
            }
        	$.extend(newRowData, newdata || {});
        	$.extend(newRowData, newColumnsDatas || {});
        	grid.beginEdit(newRowData);
        },
        /*
            ...
            else
            {
                ...
                for (var columnid in g.editors[rowdata['__id']])
                {
                    ...
                    if(column.editor["type"]=='date'){
                    	dateCols[column["columnname"]]=column["format"]||"yyyy-MM-dd";
                    }
                    if (column.name)
                        newdata[column.name] = o.editor.getValue(o.input, o.editParm);
                }
                ...
                newdata=rowdata;
                g.trigger('afterSubmitEdit', { record: rowdata, rowindex: rowdata['__index'], newdata: newdata ,dateCols :dateCols});
            }
        },
        ...
        */
        ...
        updateRow: function (rowDom, newRowData)
        {
            ...
            if (rowdata[p.statusName] != 'add')
                rowdata[p.statusName] = 'update';
            g.reRender.ligerDefer(g, 0, [{ rowdata: rowdata}]);
            return rowdata;
        },
        ...
        setColumnWidth: function (columnparm, newwidth)
        {
            ...
            //修复调整列宽后不对其的bug
            var headCol=$(document.getElementById(column['__domid']));
            var headColInner=headCol[0].children;
            headCol.css('width', newwidth);
            $(headColInner).css('width', newwidth-2);
            var cells = [];
            for (var rowid in g.records)
            {
...
//                  g.endEdit();  //除去拖动滚动条的时候编辑状态取消
        ...
        _onResize: function ()
        {
            var g = this, p = this.options;
            if(p.height=='auto'){
            	p.height="100%";//加上就有纵向滚动条      没加就有横向滚动条
            }
        ...
        _getSearchData : function(response){
        	var data=eval(response);
			var g = grid, p = grid.options;
			pageBean=data[1];
			g.currentData[p.root] = g.data[p.root] = getFormatData(data[0]);
	        g.currentData[p.record] = g.data[p.record] =pageBean['totalCount'];
	        g._showData();
