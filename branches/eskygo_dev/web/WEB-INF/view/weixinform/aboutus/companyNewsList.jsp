	...
	<body>
		...
		<ul>
			<c:forEach items="${cms}" var="cms">
				...
				<div class="messageName text-black2 textoverflow" onclick="viewDetail(${cms.siteMenuContentId});">${cms.contentTitle}</div>
					 <p>
		             	${cms.contentSummary}  
					</p> <div>${cms.contentCreateDate}<span class="text-orange2" onclick="viewDetail(${cms.siteMenuContentId});">查看</span></div>
				</li>
			</c:forEach>
		</ul> 
		</div>
		...
		function viewDetail(contentId){
			window.location.href = "${ctx}/weixin/wxaboutus/companyNewsDetails.ht?id="+contentId;
		}
		</script>
	</body>
</html>