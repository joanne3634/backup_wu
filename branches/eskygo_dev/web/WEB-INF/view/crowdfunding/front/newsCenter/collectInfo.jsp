...
<div id="bodyDiv" class="all-100 push-center">
	<form action=""   name="pageForm" id="pageForm" method="post">
		...
		<table class="PrDyBackTable ink-table bordered">
			...
			<c:forEach items="${myCollectPros }" var="myCollectPro" varStatus="s">
				<tr>
					<td><input type="checkbox" name="checkbox" value="${myCollectPro.msgRemindId }" ></td>
					<td>
						<span>${s.index+1 }</span>
					...
					<td style="max-width: 200px" title="${myCollectPro.content }">${myCollectPro.content }</td>
					...
					<td><fmt:formatDate value="${myCollectPro.sendTime}" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate></td>
					<td>
						<i class="fa fa-search-plus" title="查看" role="td-btn" data-type="view" data-msg-id="${myCollectPro.msgRemindId }"></i>
						<i class="fa fa-comment" title="回复" role="td-btn" data-type="reply" data-msg-id="${myCollectPro.msgRemindId }" data-sender-name="${myCollectPro.senderName}"></i>
						<i class="fa fa-trash-o" title="删除" role="td-btn" data-type="delete" data-msg-id="${myCollectPro.msgRemindId }"></i>
					</td>
				</tr>
			</c:forEach>
			</tbody>
