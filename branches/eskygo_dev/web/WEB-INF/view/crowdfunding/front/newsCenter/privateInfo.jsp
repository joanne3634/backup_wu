...
<div id="bodyDiv" class="all-100 push-center">
	<form action=""   name="pageForm" id="pageForm" method="post">
		...
		<table class="PrDyBackTable ink-table bordered">
			...
			<c:forEach items="${privateInfos }" var="privateInfo" varStatus="s">
				<tr>
					<td><input type="checkbox" name="checkbox" value="${privateInfo.msgRemindId }" ></td>
					<td>
						<span>${s.index+1 }</span>
					...
					<td style="max-width: 200px" title="${privateInfo.content }">${privateInfo.content }</td>
					<td>${privateInfo.replyTimes}</td>
					<td>${privateInfo.senderName}</td>
					<td><fmt:formatDate value="${privateInfo.sendTime}" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate></td>
					<td>
						<i class="fa fa-search-plus" title="查看" role="td-btn" data-type="view" data-msg-id="${privateInfo.msgRemindId }"></i>
						<i class="fa fa-comment" title="回复" role="td-btn" data-type="reply" data-msg-id="${privateInfo.msgRemindId }" data-sender-name="${privateInfo.senderName}"></i>
						<i class="fa fa-trash-o" title="删除" role="td-btn" data-type="delete" data-msg-id="${privateInfo.msgRemindId }"></i>
					</td>
				</tr>
			</c:forEach>
			</tbody>
