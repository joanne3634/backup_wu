...
<div id="bodyDiv" class="all-100 push-center">
	<form action=""   name="pageForm" id="pageForm" method="post">
		...
		<table class="PrDyBackTable ink-table bordered">
			...
			<c:forEach items="${sysInformInfos }" var="sysInformInfo" varStatus="s">
				<tr>
					<td><input type="checkbox" name="checkbox" value="${sysInformInfo.msgRemindId }" ></td>
					<td>
						<span>${s.index+1 }</span>
					...
					<td style="max-width: 200px" title="${sysInformInfo.content }">${sysInformInfo.content }</td>
					<td>${sysInformInfo.replyTimes}</td>
					<td>${sysInformInfo.senderName}</td>
					<td><fmt:formatDate value="${sysInformInfo.sendTime}" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate></td>
					<td>
						<i class="fa fa-search-plus" title="查看" role="td-btn" data-type="view" data-msg-id="${sysInformInfo.msgRemindId }"></i>
						<i class="fa fa-trash-o" title="删除" role="td-btn" data-type="delete" data-msg-id="${sysInformInfo.msgRemindId }"></i>
					</td>
				</tr>
			</c:forEach>
			</tbody>
