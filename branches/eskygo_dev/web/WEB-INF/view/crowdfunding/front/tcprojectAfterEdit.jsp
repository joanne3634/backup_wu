...
<body >
	...
	<form id="saveForm" action="${ctx}/crowdfunding/tcprojectAfter/projectAfterSave.ht" method="post">
		<input type="hidden" id=projectAfterFile value="${tcprojectAfter.fileId }">
		<div class="addPrDy">
			<div><label>项目：</label>
...
<!-- 			<c:forEach items="${arrayDataList}" var ="pr">
			${pr[1]}<img src="${ctx }/crowdfunding/Skin/Default/images/deleteIcont.png" title="删除" onclick="delFile(${pr[0]})">&nbsp;&nbsp;&nbsp;
			</c:forEach> -->
			</div>
			<div>			<input type="text" name="fileId" id="fileId" class="attach" attachType="2"
						value="${tcprojectAfter.fileId}" /></div>
...
<script type="text/javascript">
	...
	function submitPage(){
		//alert(window.parent);return false;
		...
		//alert("projectId="+projectSelect+"&paTypeId="+paTypeId+"&paTitle"=+paTitle);
			...
			$.ajax({
			url : "${ctx}/crowdfunding/tcprojectAfter/projectAfterUpdate.ht?time="+new Date(),
			type : 'post',
			data: "afterId="+${tcprojectAfter.afterId}+"&projectId="+projectSelect+"&paTypeId="+paTypeId+"&paTitle="+paTitle+
