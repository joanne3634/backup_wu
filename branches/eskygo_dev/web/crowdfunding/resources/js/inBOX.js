...
function delChecks(){
    ...
    if(str!=""){
        window.top.$.Dialog.confirm('提示','您确定删除吗', function(){
            $.ajax({
                url : _ctx+"/loan/tuserMsgRemind/delTUserMsgRemind.ht?msgRemindId="+str+"&flag="+2,
                type : 'post',
...
function delMsg(MsgRemindId){
    ...
    window.top.$.Dialog.confirm('提示','您确定删除吗', function(){
        $.ajax({
            url : _ctx+"/loan/tuserMsgRemind/delTUserMsgRemind.ht?msgRemindId="+MsgRemindId+"&flag="+2,
            type : 'post',
            success : function(data) {
                if(data=="1"){
                    window.top.$.Dialog.toast({
