(function($){
	// 操作附件访问路径
	var fileUrl = _ctx+"/platform/system/sysFile/";
...
//		td.append(link);
	...
	function attachUpload1(tdobj){
		...
		var isSingle=0;
		var url=_ctx + "/platform/system/sysFile/dialog.ht?isSingle="+isSingle;
		url=url.getNewUrl();
	 	var param = window;
	 	param.curobj = curobj;
	...
	function attachUpload2(tdobj){
		var curobj = tdobj.parent().find('input:first');
		var url=_ctx+"/platform/system/sysFile/uploadDialog.ht?typeId=0";
	 	url=url.getNewUrl();
	 	var param = window;
	 	param.curobj = curobj;
	...
	function initAttach(obj,attachs)
	{
		for(var idx=0;idx<attachs.length;idx++){
			...
			var li = $("<li><span class='dian'></span></li>");
...
//			var attachLabel = $("<span></span>").addClass('attach-label');
			...
			downlink.attr("href","javascript:download('" + attach.fileId + "');");
			downlink.html(attach.fileName);
			li.append(downlink);
//			var attachBtn = $("<span></span>");
