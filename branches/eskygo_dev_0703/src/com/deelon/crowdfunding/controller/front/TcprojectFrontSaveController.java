...
public class TcprojectFrontSaveController {
	...
	private  Logger logger = LoggerFactory.getLogger(TcprojectFrontSaveController.class);
	@ Resource
	...
	public Object sumbmitTcproject(HttpServletRequest request, HttpServletResponse response) { 
		...
		Tcproject model = tcprojectService.getById(Long.parseLong(projectId));
		...
		if(StringUtil.isNotEmpty(request.getParameter("pvideo"))){//如果是上传视频路径，即非提交项目资料操作，则不修改项目状态
			...
			Tarea areaT = null;
			Long areaTID = area.getParentId();
			if(null != area && 0 == area.getIsRecommended()){
				if("市辖区".equals(area.getAreaName()) || "县".equals(area.getAreaName())){
					//找到上一级地域
					areaT = tareaService.getById(areaTID);
					if(null != area && 0 == area.getIsRecommended()){
						areaT.setIsRecommended(1l);
						tareaService.update(areaT);
					}
				} else{
		...
		try {
			...
			logger.error("save project basic info error:errorCode = "+WuyouchouErrorCode.PRJ_BASIC_SAVE_ERROR.getErrorCode()
					+",projectId={}",projectId,e);
		throw new CrowdFundException(WuyouchouErrorCode.PRJ_BASIC_SAVE_ERROR.getErrorCode(),
				WuyouchouErrorCode.PRJ_BASIC_SAVE_ERROR.getErrorMsg(),e);
		}
...
//		}
		...
		try{
			...
			logger.error("save project error:errorCode = "+WuyouchouErrorCode.PRJ_INFO_SAVE_ERROR.getErrorCode()
						+",projectId={}",tcproject.getProjectId(),e);
			throw new CrowdFundException(WuyouchouErrorCode.PRJ_INFO_SAVE_ERROR.getErrorCode(),
					WuyouchouErrorCode.PRJ_INFO_SAVE_ERROR.getErrorMsg(),e);
		}
	...
	public ModelAndView saveprojectmodel(TcprojectModel tcprojectmodel,HttpServletRequest request, HttpServletResponse response) { 
		TcprojectModel model = null;
		logger.info("saveprojectmodel:"+tcprojectmodel.getProjectId()+"--"+tcprojectmodel.getPmtargeted()+"--"+tcprojectmodel.getPmcurrentlyPlaguing()+"--"+
		tcprojectmodel.getPmservicesMode()+"--"+tcprojectmodel.getPmprofitableDesc()+"--"+tcprojectmodel.getPmcompetitors()+"--"+tcprojectmodel.getPmcoreCompetitiveness());
		try {
			TcprojectModel tcpModel = tcprojectModelDao.getByProId(tcprojectmodel.getProjectId());
			if(null != tcpModel){//如果存在就更新
				Long modelId = tcpModel.getModelId();
				tcprojectmodel.setModelId(modelId);
				tcprojectModelDao.update(tcprojectmodel);
			} else {
				model = tprojectFrontSaveService.saveTcprojectModelInfo(tcprojectmodel);
			}
		} catch (Exception e) {
			logger.error("save projectMode error:errorCode = "+WuyouchouErrorCode.PRJ_MODEL_SAVE_ERROR.getErrorCode()
					+",projectId={}",tcprojectmodel.getProjectId(),e);
			throw new CrowdFundException(WuyouchouErrorCode.PRJ_MODEL_SAVE_ERROR.getErrorCode(),
				WuyouchouErrorCode.PRJ_MODEL_SAVE_ERROR.getErrorMsg(),e);
		}
		
		String url = "";
		if("2".equals(request.getParameter("next"))){
			url = "redirect:/crowdfunding/tcuserPersonalData/myAccount.ht?projectId="+tcprojectmodel.getProjectId();
		...
		for(Object obj:jsonArr){
			...
			if(!"".equals(json.getString("phpname"))){
				...
				try {
					...
					logger.error("save history and future project error:errorCode = "+WuyouchouErrorCode.PRJ_HISTORY_SAVE_ERROR.getErrorCode()
							+",projectId={}",projectId,e);
					throw new CrowdFundException(WuyouchouErrorCode.PRJ_HISTORY_SAVE_ERROR.getErrorCode(),
						WuyouchouErrorCode.PRJ_HISTORY_SAVE_ERROR.getErrorMsg(),e);
				}
			}
		}
		String url = "";
		...
		for(Object obj:jsonArr){
			...
			if(!"".equals(json.getString("teamId"))){
				...
				try{
					...
					logger.error("save project team error:errorCode = "+WuyouchouErrorCode.PRJ_TEAM_SAVE_ERROR.getErrorCode()
							+",projectId={}",projectId,e);
					throw new CrowdFundException(WuyouchouErrorCode.PRJ_TEAM_SAVE_ERROR.getErrorCode(),
						WuyouchouErrorCode.PRJ_TEAM_SAVE_ERROR.getErrorMsg(),e);
				}
			}
		}
		String url = "";
	...
	public Object queryTcprojectsOrInvestors(HttpServletRequest request,HttpServletResponse response) {
		...
		if(StringUtil.isEmpty(param)){
			...
			List<Tcproject> ListVo= null;
			try {
				ListVo= tcprojectService.getAllProByWhere(map,pb);
			} catch (Exception e) {
