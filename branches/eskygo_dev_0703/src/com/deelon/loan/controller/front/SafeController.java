...
public class SafeController {
	...
	public String sendPhoneValiCode(HttpServletRequest request,
		...
		try {
			if ("changephone".equals(phoneFlag)) { 
				//修改手机先验证是否输入原手机正确
				TUserSafe userSafe = userSafeService.getUserSafeByUserId(user.getUserId()); // 根据当前获取到用户的绑定手机号
				if (userSafe != null) {
			...
			//判断输入的手机号是否已被绑定
			List<TUserSafe> list = tUserSafeDao.getUserSafeByColumn("Mobile", phone);
			if (user == null) {
				...
				boolean flag = false;
				for(TUserSafe u: list){
					if(phone.equals(u.getMobile()) && u.getUserId() != user.getUserId()){
						flag = true;
					}
				}
				if(flag){
			...
			uck.setUserId(null == user ? Long.parseLong(phone) : user.getUserId()); // 如果是注册user为null，使用phone， 如果是修改绑定，user存在使用userid
			uck.setCheckCodeId(UniqueIdUtil.genId()); // 验证记录ID
			...
			tsmsinformation.setCreator(Long.parseLong(phone));// 创建人ID
	...
	public ModelAndView valiBindEmail(HttpServletRequest request,
		...
		if ((validCode == null)
			...
			return new ModelAndView("/loan/myAccount/wangjimima.jsp")
					.addObject("email", email)
					.addObject("identifying", "yxzh-btn")
					.addObject("error", "验证码输入有误");
		...
		try {
			...
			if (userSafe == null) {
				return new ModelAndView("/loan/myAccount/wangjimima.jsp").addObject("email",email)
						.addObject("identifying", "yxzh-btn").addObject("error", "该邮箱没有绑定用户名");
			}
	...
	public ModelAndView valiBindQuesion(HttpServletRequest request,
		...
		try {
			...
			if (user == null) {
				return new ModelAndView("/loan/myAccount/wangjimima.jsp").addObject("loginName", loginName)
