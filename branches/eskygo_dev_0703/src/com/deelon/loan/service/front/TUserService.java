...
public class TUserService extends BaseService<TUser> {
     ...
     * @return
     */
    public String fastRegist(TUserSafe userSafe, TUser user, HttpServletRequest request) throws Exception{
        ...
        if (tUserSafeDao.getUserSafeByColumn("Mobile", userSafe.getMobile()).size() > 0) {
            ...
            tuserFunds.setFundsId(UniqueIdUtil.genId());
            tuserFunds.setUserId(user.getUserId());
            BigDecimal limitMaxIn = null;
            try{
            	limitMaxIn = BigDecimal.valueOf(Long.valueOf(AppUtil.getServletContext().getAttribute("LIMIT_MAX_IN").toString()));
            }catch(Exception e){
            	logger.error("",e);
            	limitMaxIn = new BigDecimal(0);
            }
            tuserFunds.setLimitMaxIn(limitMaxIn);  //系统设置借入额度
            
            BigDecimal limitMaxOut = null;
            try{
            	limitMaxOut = BigDecimal.valueOf(Long.valueOf(AppUtil.getServletContext().getAttribute("LIMIT_MAX_OUT").toString()));
            }catch(Exception e){
            	logger.error("",e);
            	limitMaxOut = new BigDecimal(0);
            }
            tuserFunds.setLimitMaxOut(limitMaxOut);//系统设置借出额度
            tuserFunds.setBalance(BigDecimal.valueOf(0));
            tuserFunds.setFreeze(BigDecimal.valueOf(0));
            tuserFunds.setAvailable(BigDecimal.valueOf(0));
            tuserFunds.setFundsIn(BigDecimal.valueOf(0));
            ...
            try{
            	inviteRedPacket = AppUtil.getServletContext().getAttribute("INVITE_RED_PACKET").toString();
            }catch(Exception e){
            	logger.error("",e);
            	inviteRedPacket = "0";
            }
            
            if("1".equals(inviteRedPacket)){
            	//掉红包接口
            }
            //查询注册送红包开关
            String registRedPacket = "";
            try{
            	registRedPacket = AppUtil.getServletContext().getAttribute("REGIST_RED_PACKET").toString();
            }catch(Exception e){
            	logger.error("",e);
            	registRedPacket = "0";
            }
            if("1".equals(registRedPacket)){
            	//掉红包接口
            }
            //发送验证码到短信开始
            String content = "您好，您已成功注册为无忧筹众筹系统的会员，会员名为" + user.getLoginName() + "【无忧筹】";
     ...
     * @return
     */
	public boolean isUserDoc(TUser user) {
		String isUserDoc = user.getCAddress();
		if("1".equals(isUserDoc)){
			return true;
		}
		return false;
	}
