(function($){
	...
	$.fn.attach = function(){
		...
		var link = $('<a style="background: #8ec017;color: #fff;font-size: 14px;'+
		'margin-left: 10px;padding: 6px 13px 6px 13px;" href="#"></a>');
		link.html("添加附件");
				...
				$.ligerDialog.confirm('确认删除吗？','提示信息',function(rtn) {
					if(rtn) {
						...
						$.post(url,param,function(result){
							...
							if (obj.success)
							{
								var input = curobj.closest('ul').siblings('input');
								var attachs = getAttachs(input);
