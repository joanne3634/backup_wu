			 ...
			 if($('#sysUserForm').valid()){
				 $.ajax({
					 ...
					 success : function(data,textStatus) {
						 if(data.result == 1){
							 $.ligerDialog.alertExt('保存成功','提示信息',function(rtn){
								 if(rtn){
									 location.href = "${ctx}/crowdfunding/backstage/allProject/listUpd.ht";
								 }
		...
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<c:if test="${flag eq 'tcAfter'}">
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link back" href="${ctx}/crowdfunding/backstage/tcprojectAfterInfo/list.ht"><span></span>返回</a></div>
					</c:if>
					<c:if test="${flag eq ''||flag==null}">
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link back"
					<c:if test="${empty IsShowBtn }">href="${ctx}/crowdfunding/backstage/allProject/listUpd.ht"</c:if>
					<c:if test="${not empty IsShowBtn }">href="${IsShowBtn }"</c:if> 
					><span></span>返回</a>
					</div>
					<div class="l-bar-separator"></div>
					<c:if test="${empty IsShowBtn }">
						<%-- <div class="group"><a class="link back" href="javascript:agreeApply(${tcproject.projectId},'agree',${tcproject.isDel});"><span></span>审核通过</a></div>
						<div class="l-bar-separator"></div>
						<div class="group"><a class="link back" href="javascript:recommandProject(${tcproject.projectId},${tcproject.pisRecommended})"><span></span><c:if test="${tcproject.pisRecommended==1}">取消</c:if>推荐项目</a></div>
						<div class="l-bar-separator"></div>
						<div class="group"><a class="link back" href="javascript:agreeApply(${tcproject.projectId},'disagree');"><span></span>审核不通过</a></div> --%>
					</c:if>
					</c:if>
				</div>
			</div>
		</div>
	   <form id="sysUserForm" method="post" action="${ctx}/crowdfunding/backstage/proAudit/saveUpd.ht">
	           ...
	           <div title="项目详情" tabid="userdetail" icon="${ctx}/styles/default/images/resicon/user.gif">
			         <table class="table-detail" cellpadding="0" cellspacing="0" border="0">
							<tr>
								<th width="25%">项目名称:</th>	
								<td>										 
									<input required="required" type="text" id="pname" name="pname" value="${tcproject.pname}" class="inputText" />
								</td>
								<th width="25%">项目状态:</th>	
								<td>	
								 <f:select  nodeKey="xmzt" id="pstateId" defaultValue="${tcproject.pstateId}"></f:select>
							...
							<tr>
								...
								<td>										 
									...
									<f:select nodeKey="PTYPE" id="pstage"  defaultValue="${tcproject.pstage}"></f:select>
								</td>
								...
								<td>										 
									<%-- <input readonly="readonly"  readonly="readonly"  type="text" id="pisHaveOther" name="pisHaveOther" value="${tcproject.pisHaveOther}" class="inputText" /> --%>
										<f:select nodeKey="sf" id="pisHaveOther" defaultValue="${tcproject.pisHaveOther}"></f:select>
										<%--<c:choose>
							...
							<tr>
								...
								<td>										 
									<f:select  styleClass="inputText" nodeKey="PIND" id="pindustry" idName="pindustry" defaultValue="${tcproject.pindustry}" showBlankOnTop="true" ></f:select>
								</td>
								<th width="25%">融资金额（万元）:</th>	
								<td>										 
									<input  type="text" id="pfinancingAmount" name="pfinancingAmount" value="<fmt:formatNumber value="${tcproject.pfinancingAmount/10000 }" pattern="####.##"  minFractionDigits="0"  ></fmt:formatNumber>" class="inputText" />
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>										 
									<%-- <input readonly="readonly"  type="text" id="psetup" name="psetup" value="${tcproject.psetup}" class="inputText" /> --%>
										<f:select nodeKey="sf" id="psetup" defaultValue="${tcproject.psetup}"></f:select>
									<%--<c:choose>
							...
							<tr>
								...
								<th width="25%">结束日期:</th>	
								<td>										 
									<input  type="text" name="pcloseDate" id="pcloseDate" value="<fmt:formatDate value='${tcproject.pcloseDate}' />" class="inputText date" style="width:110px;"/>
								</td>
							</tr>
							<tr>
								<th width="25%">是否推荐项目:</th>	
								<td>
									<f:select nodeKey="sf" id="pisRecommended" defaultValue="${tcproject.pisRecommended}"></f:select>
									<%--<c:choose>
							...
							<tr>
								...
								<td>										 
									...
									<textarea rows="2" cols="1" id="paddress" name="paddress">${tcproject.paddress}</textarea>
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<th width="25%">项目估值:</th>	
								<td>										 
									<input  type="text" id="pvaluationFinal" name="pvaluationFinal" value="<fmt:formatNumber value="${tcproject.pvaluationFinal}" pattern="#####.##"  minFractionDigits="0"  />" class="inputText" />万
								</td>
								
								<th width="25%">企业成立年份:</th>
								<td>										 
									<input  readonly="readonly"  type="text" id="pfoundedYear" name="pfoundedYear" value="${tcproject.pfoundedYear}" class="inputText" />
								</td>
							</tr>
							<tr>
								...
								<td colspan="4">										 
									...
									<textarea rows="2" cols="1"  id="pdetail" name="pdetail">${tcproject.pdetail}</textarea>
								</td>
